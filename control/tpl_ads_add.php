...
if($_GET['insert'] == "true") {
	...
	$mysql_class->insert("slider", $data);
	$thisid = $mysql_class->insert_id();
	$url = move_attachment($data['picture'], "slider/{$thisid}/".basename($data['picture']));
	if($url) {
		$mysql_class->update("slider", array("picture"=>$url), array("id"=>$thisid));
	}
	adminlog("添加了图片乱换（ID：".$thisid."）");
...
<div class="main">
	...
	<div class="mainright">
		...
		<form action="./?ac=ads&do=add&insert=true&sid=<?php echo SID;?>" method="post" onSubmit="return check_form();">
			...
			<table class="form" >
                ...
                <tr>
					...
					<td>
						<input type="number" min="0" class="input" style="width:100px;" name="index_order">&nbsp;&nbsp;<span class="gray">数字越小越靠前</span>
					</td>
				</tr>
                <tr>
