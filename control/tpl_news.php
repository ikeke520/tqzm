...
switch($do) {
	case 'edit':
		include("./tpl_news_edit.php");
		exit;
	break;
	case 'add':
		include("./tpl_news_add.php");
		exit;
	break;
	case 'category':
		include("./tpl_news_category.php");
		exit;	
	break;
	case 'del':
		$mysql_class->delete("news", "id IN ('".implode("','", explode(",", trim($_GET['id'])))."')");
		$thisnews = $mysql_class->select("news", "id IN ('".implode("','", explode(",", trim($_GET['id'])))."')");
		if($thisnews) {
			foreach($thisnews as $list) {
				remove_file(check_dir(dirname(STCMS_ROOT))."attachment/news/{$list['id']}");
			}
		}
		adminlog("删除资讯：".trim($_GET['id']));
	...
	case 'upload':
		...
		if(@move_uploaded_file($filesrc['tmp_name'], $upload_root.$path_subfix)) {
			imgresize($upload_root . $path_subfix, 151, 120);
...
$total_records = $mysql_class->num_table("news", $where);
$page_class = load_class('page');
$page_class->init($page, $total_records, $per_page);
$records = $mysql_class->select("news", "*", $where, "id DESC", array(($page_class->page-1)*$per_page, $per_page));
?>
<!doctype html>
<html>
<head>
<meta charset="utf-8">
<title>资讯管理__后台管理中心__<?php echo $config['seo_title'];?></title>
...
<div class="main">
	<div class="position">您的位置：<a href="../" target="_blank"><?php echo $config['seo_title'];?></a>&nbsp;&raquo;&nbsp;<a href="./">后台管理中心</a>&nbsp;&raquo;&nbsp;资讯管理</div>
	<div class="mainright">
		<div class="tab">
			<ul>
				<li class="on"><a href="./?ac=news">资讯列表</a></li>
...
?>
						<tr>
							...
							<td><a href="./?ac=news&do=edit&id=<?php echo $list['id'];?>"><?php echo $list['title'];?></a></td>
							<td><?php echo $list['cname'];?></td>
							<td><span class="red"><?php echo $list['hit'];?></span></td>
							<td><?php echo $list['time'];?></td>
							<td><a href="./?ac=news&do=edit&id=<?php echo $list['id'];?>">修改</a>&nbsp;&nbsp;&nbsp;&nbsp;<a onClick="SU.dialog({title:'操作确认', 'msg':'你确定要删除该资讯吗？', cb:function(){location='./?ac=news&do=del&id=<?php echo $list['id']; ?>';}});" href="javascript:">删除</a></td>
						</tr>
<?php
	}
}
