...
switch($type) {
	case 0:
		$table = "cardbase";
		$where .= " AND wid='{$thisworker['id']}' AND is_use = '1' AND sell_time >= '{$startdate} 00:00:00' AND sell_time <= '{$enddate} 23:59:59'";
	break;
	case 1:
		$table = "users_paylog";
		$where .= " AND uid IN (SELECT uid FROM {$config['db_prefix']}users_relation WHERE pid='{$thisworker['uid']}') AND ctype = '0' AND time >= '{$startdate} 00:00:00' AND time <= '{$enddate} 23:59:59' AND is_payed = '1' AND is_refund='0'";
	break;
	case 2:
		$table = "users_paylog";
		$where .= " AND uid IN (SELECT uid FROM {$config['db_prefix']}users_relation WHERE pid='{$thisworker['uid']}') AND ctype = '1' AND time >= '{$startdate} 00:00:00' AND time <= '{$enddate} 23:59:59' AND is_payed = '1' AND is_refund='0'";
	break;
}
	...
	<div class="mainright">
		...
		<div class="mt10">
            <div class="tabcard">
                <div class="menu">
                    <ul>
                        <li><a href="./?ac=sale&do=worker&step=overview&id=<?php echo $thisworker['id'];?>&startdate=<?php echo $startdate;?>&enddate=<?php echo $enddate;?>">总括</a></li>
						<li class="on"><a href="./?ac=sale&do=worker&step=list&id=<?php echo $thisworker['id'];?>&startdate=<?php echo $startdate;?>&enddate=<?php echo $enddate;?>">明细</a></li>
                        <div class="clear"></div>
                    </ul>
                </div>
            </div>
		...
		<div class="mt10 tc">
            <a class="button" href="./?ac=sale&do=worker&step=list&id=<?php echo $thisworker['id'];?>&startdate=<?php echo $startdate;?>&enddate=<?php echo $enddate;?>&type=0">实体卡订单</a>
            &nbsp;&nbsp;<a class="button" href="./?ac=sale&do=worker&step=list&id=<?php echo $thisworker['id'];?>&startdate=<?php echo $startdate;?>&enddate=<?php echo $enddate;?>&type=1">线上现金订单</a>
            &nbsp;&nbsp;<a class="button" href="./?ac=sale&do=worker&step=list&id=<?php echo $thisworker['id'];?>&startdate=<?php echo $startdate;?>&enddate=<?php echo $enddate;?>&type=2">线上积分订单</a>
		</div>
<?php
if($type == 0) {
?>
