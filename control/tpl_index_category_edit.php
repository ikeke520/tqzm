...
$thiscategory = $mysql_class->select_one("category", "*", array("id"=>intval($_GET['id'])));
...
if($_GET['update'] == "true") {
	...
	if($thiscategory['url'] != $data['url']) {
		$data['url'] = move_attachment($data['url'], "category/{$thiscategory['id']}/".basename($data['url']));
		remove_attachment($thiscategory['url']);
	}
	$mysql_class->update("category", $data, array("id"=>$thiscategory['id']));
	adminlog("修改活动分类：".trim($thiscategory['id']));
...
<head>
<meta charset="utf-8">
<title>修改活动分类__后台管理中心__<?php echo $config['seo_title'];?></title>
...
<div class="main">
	...
	<div class="mainright">
		...
		<form action="./?ac=index&do=category&step=edit&update=true&id=<?php echo $thiscategory['id'];?>&sid=<?php echo SID;?>" method="post" onSubmit="return check_form();">
			<input type="hidden" name="url" value="<?php echo $thiscategory['url'];?>">
            <table class="form" >
				<tr>
					<td height="40" align="right"><span class="red">*</span>分类名称：</td>
					<td>
						<input type="text" class="input" name="name" value="<?php echo $thiscategory['name'];?>">
					...
					<td>
						<div class="headimguploader">
							<div id="picture" class="filelist">
								<div class="file">
									<div class="preview"><img src="<?php if($thiscategory['url']) {?><?php echo $thiscategory['url'];?><?php } else {?>../images/nophoto.jpg<?php }?>" /></div>
									<div class="process">
										<div class="parent">
											<div class="sun"></div>
										</div>
