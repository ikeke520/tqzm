...
switch($_GET['step']) {
	case 'edit':
		include("./tpl_card_base_edit.php");
		exit;
	break;
	case 'add':
		include("./tpl_card_base_add.php");
		exit;
	break;
	case 'import':
		include("./tpl_card_base_import.php");
		exit;	
	break;
	case 'upload':
		$key = trim($_GET['key']);
		...
		$path_subfix = "temp/import/".date('ymd').date('His').rand(100, 999).".".get_file_ext($filesrc['name']);
	...
	break;
	case 'del':
		$mysql_class->delete("cardbase", "id IN ('".implode("','", explode(",", trim($_GET['id'])))."')");
		adminlog("删除了实体卡（ID：".trim($_GET['id'])."）");
...
if(empty($keyword) == false) {
	$where .= " AND (name LIKE '%".$keyword."%' OR price '%".$keyword."%' OR num '%".$keyword."%' OR cardno '%".$keyword."%')";
}
switch($type) {
	case 1:
		$where .= " AND is_use = '0'";
	...
	break;
	case '3':
		$where .= " AND is_use = '1' AND is_active='0'";
	break;
	case '4':
		$where .= " AND is_active='1'";
	break;
}
$total_records = $mysql_class->num_table("cardbase", $where);
$page_class = load_class('page');
$page_class->init($page, $total_records, $per_page);
$records = $mysql_class->select("cardbase", "*", $where, "id DESC", array(($page_class->page-1)*$per_page, $per_page));
?>
<!doctype html>
<html>
<head>
<meta charset="utf-8">
<title>实体卡管理__后台管理中心__<?php echo $config['seo_title'];?></title>
...
<div class="main">
	<div class="position">您的位置：<a href="../" target="_blank"><?php echo $config['seo_title'];?></a>&nbsp;&raquo;&nbsp;<a href="./">后台管理中心</a>&nbsp;&raquo;&nbsp;<a href="./?ac=card">亲子卡管理</a>&nbsp;&raquo;&nbsp;实体卡管理</div>
	<div class="mainright">
		<div class="tab">
			<ul>
		...
		<div class="mt10">
            <div class="tabcard">
                <div class="menu">
                    <ul>
                        <li class="on"><a href="./?ac=card&do=base">实体卡列表</a></li>
...
?>
						<tr>
							...
							<td><?php if($list['is_use']) {?>已出售<?php } else {?>未出售<?php }?></td>
							<td><?php if($list['is_active']) {?>已激活<?php } else {?>未激活<?php }?></td>
							<td><?php echo $list['time'];?></td>
                            <td><a href="./?ac=card&do=base&step=edit&id=<?php echo $list['id'];?>">修改</a>&nbsp;&nbsp;&nbsp;&nbsp;<a onClick="SU.dialog({title:'操作确认', 'msg':'你确定要删除该实体卡吗？', cb:function(){location='./?ac=card&do=base&step=del&id=<?php echo $list['id']; ?>';}});" href="javascript:">删除</a></td>
						</tr>
<?php
	}
}
