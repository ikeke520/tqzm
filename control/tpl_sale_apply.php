...
switch($_GET['step']) {
	case 'edit':
		include("./tpl_sale_apply_edit.php");
		exit;
	break;
	case 'setread':
		$thisapply = $mysql_class->select_one("apply", "*", array("id"=>intval($_GET['id'])));
		if(empty($thisapply)) {
			exit(json_encode(array("error"=>1, "msg"=>"未找到指定的申请信息。")));
		}
		$mysql_class->update("apply", array("is_read"=>1), array("id"=>$thisapply['id']));
...
$total_records = $mysql_class->num_table("apply", $where);
$page_class = load_class('page');
$page_class->init($page, $total_records, $per_page);
$records = $mysql_class->select("apply", "*", $where, "id DESC", array(($page_class->page - 1)*$per_page, $per_page));
?>
<!doctype html>
<html>
<head>
<meta charset="utf-8">
<title>大客户留言__后台管理中心__<?php echo $config['seo_title'];?></title>
	...
	<div class="mainright">
		...
		<div class="mt10">
            <div class="tabcard">
                <div class="menu">
                    <ul>
                        <li class="on"><a href="./?ac=sale&do=apply">留言列表</a></li>
						<li><a href="./?ac=sale&do=apply&step=edit">留言设置</a></li>
                        <div class="clear"></div>
                    </ul>
                </div>
            </div>
...
?>
						<tr>
							...
							<td><?php if($list['is_read']) {?><span class="green">已读</span><?php } else {?><a href="javascript:" onClick="setread('<?php echo $list['id'];?>');" class="button"><span>标记已读</span></a><?php }?></td>
						</tr>
<?php
	}
}
...
function setread(id) {
	$.ajax({
		url:"./?ac=sale&do=apply&step=setread&id="+id,
		type:"GET",
		dataType:"json",
