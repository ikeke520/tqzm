...
$total_records = $mysql_class->num_table("users_coinlog", $where);
$page_class = load_class('page');
$page_class->init($page, $total_records, $per_page);
$records = $mysql_class->select("users_coinlog", "*", $where, "id DESC", array(($page_class->page-1)*$per_page, $per_page));
?>
<!doctype html>
<html>
<head>
<meta charset="utf-8">
<title>积分明细__后台管理中心__<?php echo $config['seo_title'];?></title>
...
<div class="main">
	<div class="position">您的位置：<a href="../" target="_blank"><?php echo $config['seo_title'];?></a>&nbsp;&raquo;&nbsp;<a href="./">后台管理中心</a>&nbsp;&raquo;&nbsp;<a href="./?ac=user">用户管理</a>&nbsp;&raquo;&nbsp;<a href="./?ac=user&do=edit&id=<?php echo $thisuser['id'];?>">用户详情</a>&nbsp;&raquo;&nbsp;积分记录</div>
	<div class="mainright">
		<div class="tab">
			<ul>
            ...
            <div class="tabcard">
                <div class="menu">
                    <ul>
                        ...
                        <li class="on"><a href="./?ac=user&do=edit&step=coin&id=<?php echo $thisuser['id'];?>">积分记录</a></li>
                        <div class="clear"></div>
                    </ul>
                </div>
            </div>
        ...
        <div class="mt10">
			<div class="thead">
				<form id="search-form" method="get" action="./">
                    ...
                    <input type="hidden" name="step" value="coin">
					...
					&nbsp;&nbsp;<a href="javascript:" onclick="exchange();" class="button">积分兑换</a>
...
?>
                    <tr>
                        ...
                        <td><?php if($list['type'] == 0){?>奖励/返还<?php } else if($list['type'] == 1) {?>消费<?php } else if($list['type'] == 2) {?>积分兑换<?php }?></td>
                        <td><?php if($list['type'] == 0){?><span class="green">+<?php echo $list['coin'];?></span><?php } else if($list['type'] == 1) {?><span class="red">-<?php echo $list['coin'];?></span><?php } else if($list['type'] == 2) {?><span class="red">-<?php echo $list['coin'];?></span><?php }?></td>
