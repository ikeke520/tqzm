...
if($_GET['step'] == "true") {
	...
	$mysql_class->insert("card", $data);
	$thisid = $mysql_class->insert_id();
	$url = move_attachment($data['url'], "card/{$thisid}/".basename($data['url']));
	if($url) {
		$mysql_class->update("card", array("url"=>$url), array("id"=>$thisid));
	}
	adminlog("添加了亲子卡（ID：".$thisid."）");
...
<head>
<meta charset="utf-8">
<title>新增亲子卡__后台管理中心__<?php echo $config['seo_title'];?></title>
...
<div class="main">
	<div class="position">您的位置：<a href="../" target="_blank"><?php echo $config['seo_title'];?></a>&nbsp;&raquo;&nbsp;<a href="./">后台管理中心</a>&nbsp;&raquo;&nbsp;<a href="./?ac=card">亲子卡管理</a>&nbsp;&raquo;&nbsp;新增亲子卡</div>
	<div class="mainright">
		<div class="tab">
			<ul>
		...
		<form action="./?ac=card&do=add&step=true&sid=<?php echo SID;?>" method="post" onSubmit="return check_form();">
			...
			<table class="form" >
				...
				<tr>
					...
					<td>
						<input type="text" class="input" name="org_price" value="" style="width:100px;">
				...
				<tr>
					...
					<td>
						<input type="number" min="1" class="input" name="num" value="" style="width:100px;">
				...
				<tr>
					...
					<td>
						<input type="number" min="1" class="input" name="index_order" value="" style="width:100px;">
					...
					<td>
						<div class="imguploader">
							<div id="picture" class="filelist">
								<div class="file">
									<div class="preview"><img src="<?php if($setting['banner']) {?><?php echo $setting['banner'];?><?php } else {?>../images/nopreview.png<?php }?>" /></div>
									<div class="process">
										<div class="parent">
											<div class="sun"></div>
										</div>
				...
				<tr>
					...
					<td>
						<textarea class="textarea" name="intro" style="width:500px; height:200px;"></textarea>
					</td>
				</tr>
				<tr>
