...
$thiscard = $mysql_class->select_one("cardbase", "*", array("id"=>intval($_GET['id'])));
if(empty($thiscard)) {
	halt("系统未找到指定的实体卡。");
}
if($_GET['update'] == "true") {
	...
	$mysql_class->update("cardbase", $data, array("id"=>$thiscard['id']));
	adminlog("修改了实体卡（ID：".$thiscard['id']."）");
...
<head>
<meta charset="utf-8">
<title>修改实体卡__后台管理中心__<?php echo $config['seo_title'];?></title>
...
<div class="main">
	<div class="position">您的位置：<a href="../" target="_blank"><?php echo $config['seo_title'];?></a>&nbsp;&raquo;&nbsp;<a href="./">后台管理中心</a>&nbsp;&raquo;&nbsp;<a href="./?ac=card">亲子卡管理</a>&nbsp;&raquo;&nbsp;修改实体卡</div>
	<div class="mainright">
		<div class="tab">
			<ul>
            ...
            <div class="tabcard">
                <div class="menu">
                    <ul>
						...
						<li class="on"><a href="javascript:">编辑实体卡</a></li>
                        <div class="clear"></div>
                    </ul>
                </div>
            </div>
		<form action="./?ac=card&do=base&step=edit&update=true&id=<?php echo $thiscard['id'];?>&sid=<?php echo SID;?>" method="post" onSubmit="return check_form();">
			<table class="form" >
<?php
if($thiscard['is_use']) {
?>
                <tr>
					<td height="40" align="right">亲子卡状态：</td>
					<td>
						已销售<span class="gray"></span>
					</td>
				</tr>
                <tr>
					<td height="40" align="right">销售员：</td>
					<td>
						<a href="./?ac=sale&do=worker&step=edit&id=<?php echo $thiscard['wid'];?>"><?php echo $thiscard['worker'];?></a>
					</td>
				</tr>
                <tr>
					<td height="40" align="right">购卡人：</td>
					<td>
						<a href="./?ac=user&do=edit&id=<?php echo $thiscard['uid'];?>"><?php echo $thiscard['uname'];?></a>
					</td>
				</tr>
                <tr>
					<td height="40" align="right">卖出时间：</td>
					<td>
						<?php echo $thiscard['sell_time'];?>
					</td>
				</tr>
                <tr>
					<td height="40" align="right">是否激活：</td>
					<td>
						<?php if($thiscard['is_active']) {?>已激活<?php } else {?>未激活<?php }?>
					</td>
				</tr>
                <tr>
					<td height="40" align="right">激活时间：</td>
					<td>
						<?php echo $thiscard['active_time'];?>
					</td>
				</tr>
<?php
}
?>
				...
				<tr>
					...
					<td>
						<input type="text" class="input" name="cardno" value="<?php echo $thiscard['cardno'];?>">
				...
				<tr>
					...
					<td>
						<input type="text" class="input" name="hash" value="<?php echo $thiscard['hash'];?>">
						&nbsp; <span class="gray">最长16位数字与字母组合，不区分大小写</span>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						<input type="text" class="input" name="num" value="<?php echo $thiscard['num'];?>" style="width:100px;">
