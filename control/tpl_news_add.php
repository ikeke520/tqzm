...
if($_GET['step'] == "true") {
	...
	$mysql_class->insert("news", $data);
	$thisid = $mysql_class->insert_id();
	$url = move_attachment($data['url'], "news/{$thisid}/".basename($data['url']));
	if($url) {
		$mysql_class->update("news", array("url"=>$url), array("id"=>$thisid));
	}
	adminlog("添加资讯：".trim($thisid));
...
<head>
<meta charset="utf-8">
<title>添加资讯__后台管理中心__<?php echo $config['seo_title'];?></title>
...
<div class="main">
	<div class="position">您的位置：<a href="../" target="_blank"><?php echo $config['seo_title'];?></a>&nbsp;&raquo;&nbsp;<a href="./">后台管理中心</a>&nbsp;&raquo;&nbsp;<a href="./?ac=news">资讯管理</a>&nbsp;&raquo;&nbsp;发布资讯</div>
	<div class="mainright">
		<div class="tab">
			<ul>
				<li><a href="./?ac=news">资讯列表</a></li>
				<li class="on"><a href="./?ac=news&do=add">发布资讯</a></li>
                <li><a href="./?ac=news&do=category">资讯分类</a></li>
				<div class="clear"></div>
			</ul>
		</div>
		<form action="./?ac=news&do=add&step=true&sid=<?php echo SID;?>" method="post" onSubmit="return check_form();">
...
?>
				...
				<tr>
					...
					<td>
						<input type="text" class="input" name="hit" value="0" style="width:100px;">
				...
				<tr>
					...
					<td>
						<script id="content" name="content" type="text/plain" style="width:650px; height:300px;"></script>
					</td>
				</tr>
				<tr>
