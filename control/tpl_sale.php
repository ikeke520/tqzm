...
switch($do) {
	case 'worker':
		include("./tpl_sale_worker.php");
		exit;
	break;
	case 'salary':
		include("./tpl_sale_salary.php");
		exit;
	break;
	case 'getuser':
		include("./tpl_sale_getuser.php");
		exit;
	break;
	case 'apply':
		include("./tpl_sale_apply.php");
		exit;
	break;
}
...
switch($_GET['step']) {
	case 'list':
		include("./tpl_sale_index_list.php");
		exit;
	break;
	case 'rank':
		include("./tpl_sale_index_rank.php");
		exit;
	break;
}

$tmpoffline = $mysql_class->fetch($mysql_class->query("SELECT SUM(price) AS total_price, COUNT(*) AS total_num FROM {$config['db_prefix']}cardbase WHERE sell_time >= '{$startdate} 00:00:00' AND sell_time <= '{$enddate} 23:59:59' AND is_use='1'"));
$tmponline = $mysql_class->fetch($mysql_class->query("SELECT SUM(money) AS total_price, COUNT(*) AS total_num FROM {$config['db_prefix']}users_paylog WHERE time >= '{$startdate} 00:00:00' AND time <= '{$enddate} 23:59:59' AND ctype='0' AND is_payed='1' AND is_refund='0'"));
$tmpcoin = $mysql_class->fetch($mysql_class->query("SELECT SUM(coin) AS total_coin, COUNT(*) AS total_num FROM {$config['db_prefix']}users_paylog WHERE time >= '{$startdate} 00:00:00' AND time <= '{$enddate} 23:59:59' AND ctype='1' AND is_payed='1' AND is_refund='0'"));
?>
<!doctype html>
<html>
<head>
	...
	<div class="mainright">
		...
		<div class="mt10">
            <div class="tabcard">
                <div class="menu">
                    <ul>
                        <li class="on"><a href="./?ac=sale&startdate=<?php echo $startdate;?>&enddate=<?php echo $enddate;?>">总括</a></li>
