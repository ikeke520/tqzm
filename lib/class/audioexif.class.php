...
// Author: Dahongy <dahongy@gmail.com>
class audioexif_class
{
  // public vars
  var $_wma = false;
  var $_mp3 = false;
  ...
  function GetInfo($file)
  {
    $handler = &$this->_get_handler($file);
    if (!$handler) return false;
    $data = $handler->get_info($file);
	$this->_tagsize = $handler->_tagsize;
	return $data;
  }
...
class _Mp3Exif extends _AudioExif
{
  ...
  function _init_header()
  {
    ...
    if (strlen($buf) == 10 && substr($buf, 0, 3) == 'ID3')
    {
	  ...
	  $this->_tagsize = $this->_tagsize + 10 + $tmp['size'];
    }
  ...
  function _get_v2_info()
  {
    ...
    $items = array('TCOP'=>'Copyright', 'TPE1'=>'Artist', 'TIT2'=>'Title', 'TRCK'=> 'Track',
            'TCON'=>'Genre', 'COMM'=>'Description', 'TYER'=>'Year', 'TALB'=>'AlbumTitle', 'APIC'=>'picture');
    while (true)
    {
